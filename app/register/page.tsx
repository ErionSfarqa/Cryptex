    ...
    try {
      ...
      setError(
        message === "Failed to fetch"
          ? "Failed to reach Supabase. Check NEXT_PUBLIC_SUPABASE_URL / NEXT_PUBLIC_SUPABASE_ANON_KEY and restart the dev server."
          : message
      );
    } finally {
      setLoading(false);
    ...
    <div className="page-bg min-h-screen px-6 py-16">
      <div className="mx-auto w-full max-w-lg rounded-3xl border border-[var(--border)] bg-[var(--panel)] p-8 shadow-sm">
        ...
        <p className="mt-2 text-sm text-[var(--muted)]">
          Get a starting balance and start trading instantly.
        </p>
        ...
        <div className="mt-6 border-t border-[var(--border)] pt-6">
          <Button
            type="button"
            variant="secondary"
            className="w-full gap-2 justify-center transition-colors hover:bg-[var(--panel-strong)] hover:opacity-90 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--accent)] focus-visible:ring-offset-2 focus-visible:ring-offset-[var(--panel)] active:opacity-95"
            onClick={onGoogle}
          >
